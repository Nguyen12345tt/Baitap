/*NguyenThanhNguyen_BinaryTree_V2.cpp*/
#include<iostream>
using namespace std;
typedef int Item;
...
int Dem_So_Nut_1_Con(Tree T); //Đếm và trả về nút có 1 con
int Dem_So_Nut_Cay_Con_Phai(Tree T); //Đếm và trả về nút có bao nhiêu cây con phải
int Dem_So_Nut_Cay_Con_Trai(Tree T); //Đếm và trả về nút có bao nhiêu cây con trái
Item TimSoNhoNhat(Tree T); //Tìm giá trị nhỏ nhất trong cây
Item TimSoLonNhat(Tree T); //Tìm giá trị lớn nhất trong cây
int main()
{
    Tree T;
    Menu(T);
...
Item TimSoLonNhat(Tree T) //Tìm giá trị lớn nhất trong cây
{
    if (T == NULL) //Nếu cây rỗng 
        return -1;
    while (T->right!=NULL) //Cây không rỗng thì lặp lại đến khi tìm được số lớn nhất
    {
        T =T->right;
    }
    return T->Data;
...
Item TimSoNhoNhat(Tree T) //Tìm giá trị nhỏ nhất trong cây
{
    if (T == NULL)
        return -1;
    while (T->left!=NULL) //Cây không rỗng thì lặp lại đến khi tìm được số nhỏ nhất
    {
        T =T->left;
    }
    return T->Data;
...
int Dem_So_Nut_Cay_Con_Trai(Tree T) //Đếm và trả về nút có bao nhiêu cây con trái
{
    ...
    {
        int d = Dem_So_Nut_Cay_Con_Trai(T->left)+Dem_So_Nut_Cay_Con_Trai(T->right);
        if (T->left == NULL && T->right!=NULL) //Nếu trái = NULL và phải !=NULL
            return d+1;
        else    //Ngược lại 
            return d;
    }
...
int Dem_So_Nut_Cay_Con_Phai(Tree T) //Đếm và trả về nút có bao nhiêu cây con phải
{
    if (T!=NULL)//Cây không rỗng
    {
        int d = Dem_So_Nut_Cay_Con_Phai(T->left)+Dem_So_Nut_Cay_Con_Phai(T->right);
        if (T->left != NULL && T->right == NULL) //Nếu trái != NULL và phải ==NULL
            return d+1;
        else    //Ngược lại 
            return d;
...
int Dem_So_Nut_1_Con(Tree T) //Đếm số nút 1 con của cây T
{
    if (T != NULL) //Cây không rỗng
    {
        int d = Dem_So_Nut_1_Con(T->left) + Dem_So_Nut_1_Con(T->right);//Cộng 2 số của nút con bên trái hoặc bên phải
        if ((T->left != NULL && T->right == NULL) || (T->left == NULL && T->right != NULL))
            // Kiểm tra xem nút hiện tại (T) có phải là nút 1 con hay không
            return d + 1;
        else
            return d;
            // Nếu nút T không phải là nút 1 con, trả về d (số nút 1 con trong các cây con)
    } 
    else //Nếu cây rỗng thì không có nút
        return 0;
}
int Dem_So_Nut_2_Con(Tree T) //Đếm và trả về nút có 2 con
{
...
void Menu(Tree T)//Menu các chức năng của cây
{
	...
	do
	{
        ...
        cout<<"9. Đếm số nút có 1 con\n";
        cout<<"10. Đếm số nút có 1 con bên phải\n";
        cout<<"11. Tìm giá trị nhỏ nhất của cây\n";
        cout<<"12. Tìm giá trị lớn nhất của cây\n";
		cout<<"0. Thoát\n";
		cout<<"---------------------------------------\n";
		...
		switch(LuaChon)
		{
            ...
            case 9: //Đếm số nút có 1 con
            {
                cout<<"Số nút có 1 con là:"<<Dem_So_Nut_1_Con(T)<<endl;
                break;
            }
            case 10: //Đếm số nút có bao nhiêu cây con bên phải
            {
                cout<<"Số nút có cây con bên phải là:"<<Dem_So_Nut_Cay_Con_Phai(T)<<endl;
                break;
            }
            case 11: //Đếm số nút có bao nhiêu cây con bên trái
            {
                cout<<"Số nút có cây con bên trái là:"<<Dem_So_Nut_Cay_Con_Trai(T)<<endl;
                break;
            }
            case 12: //Tìm giá trị nhỏ nhất của cây
            {
                cout<<"Giá trị nhỏ nhất của cây là:"<<TimSoNhoNhat(T)<<endl;
                break;
            }
            case 13: //Tìm giá trị lớn nhất của cây
            {
                cout<<"Giá trị lớn nhất của cây là:"<<TimSoLonNhat(T)<<endl;
                break;
            }
			case 0: //Thoát
			{
